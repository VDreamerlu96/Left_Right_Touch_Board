...
const char* global_font_name = "12x24";         /*字体,系统要有，不然会打开失败，而且最好根据当前屏幕选一个合适大小的*/
...
void Board_Main() {
    ...
    uint16_t left_board_width = screen_width/10;                /*1/10宽度*/
    uint16_t left_board_height = screen_height/4*3;             /*3/4高*/
    uint16_t left_board_x = 0;                                  /*屏幕边缘*/
    uint16_t left_board_y = screen_height/4;                    /*1/4屏幕高*/
    
    uint16_t right_board_width = left_board_width*2;            /*1/10宽度*/
    ...
    while ((event = xcb_wait_for_event(conn))) {
        switch (event->response_type & ~0x80) {
            ...
            case XCB_BUTTON_RELEASE:
                ...
                long press_duration = (release_time.tv_sec - key_state.press_time.tv_sec) * 1000 +
                              (release_time.tv_nsec - key_state.press_time.tv_nsec) / 1000000; // 毫秒计算

            if (press_duration >= LONG_PRESS_THRESHOLD) {
                touch_longpress_key_event_win(ufd, key_state.key_code); // 触发长按
            } else {
                touch_key_event_win(ufd, key_state.key_code); // 触发普通按键
            }
            key_state.is_pressed = 0; // 重置状态
    }
    break;
            default:
                break;
